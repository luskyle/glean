...
划词 / 摘录 → 收藏到拾忆。数据写入**你自己配置的 WebDAV 云盘**（坚果云等），
桌面与移动端打开拾忆会自动合并进收件箱——服务器零存储，与 App 共用同一份快照契约。
...
- **划词 → 右键 → 「收藏到拾忆」**：立即收藏（默认未分类，明天开始复习）
- **划词 → 右键 → 「收藏到拾忆（选分类…）」**：打开弹窗选择分类后收藏
- 点击浏览器工具栏的拾忆图标：手动输入内容 + 选分类收藏
...
- 云端文件：`<WebDAV 地址>/shiyi/backup.json`（与 App 云盘同步同一文件）
...
mkdir -p /tmp/shiyi-dav && wsgidav -r /tmp/shiyi-dav --host 127.0.0.1 --port 8080 --auth anonymous
