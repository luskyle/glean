...
originalUrl/sourceTitle/htmlClip/mediaType/coverUrl/note/lang/status/createdAt），
**未知字段（新增字段）安全忽略**——插件可以在 items 上自由扩展字段，App 旧版本不受影响，新版本逐步消费。

> **实施状态（2026-09-11）**：V1（链接/选区/图片）与 V1.1（视频·音频/文件、超体积询问）
> 已落地；V2 整页离线首版已落地（资源内联限额 + 失败降级）。剩余：离线阅读器、媒体孤儿清理、C 档适配器。
...
```
右键收藏 → 组装元数据（+可选抓取文件）
  → 超过体积？→ 打开弹窗询问「收藏源文件 / 仅收藏链接」（默认自动降级仅链接）
  → MKCOL /glean/media（首次）→ PUT 媒体文件 → PUT backup.json
  → ping 127.0.0.1:9797 → 桌面端静默合并
```

**超体积询问（V1.1）**：contextMenus.onClicked 无法弹对话框，方案是把待确认收藏
存入 `storage.pendingOversize` 并 `chrome.action.openPopup()` —— 弹窗即确认界面；
用户选「收藏源文件」后以 500MB 绝对上限重试上传，选「仅收藏链接」立即写引用条目。
阈值：图片/视频/音频 50MB，普通文件 100MB。
...
- 视频/音频引用收藏（封面 = og:image）＋ 可直接下载的媒体直传
- PDF 等任意可下载文件的直传；**超过 50/100MB 时弹窗询问「收藏源文件 / 仅收藏链接」**（§4.3）
- App 端消费 `htmlClip` / `mediaPath` / `coverUrl`：DB 迁移（schema v6）、合并白名单、
  详情页选区快照只读渲染 + og 封面 + 来源标签

### 6.3 V2（首版 ✅ 已落地，剩余见下）

**整页离线归档（首版）**：右键「整页离线收藏到 Glean」→ 抓取页面 HTML，将
`<img>` / `<video poster>` / `<source>` 静态资源下载并以 base64 内联为**单文件 HTML**，
上传 `/glean/media/<id>.html`（条目 `mediaType: html`）。stylesheets/scripts 保留原引用
（离线打开时静默降级）。限额：资源 ≤40 个、单个 ≤4MB、总内联 ≤15MB、页面 ≤8MB。
任何失败自动降级为「网页元数据收藏」，收藏动作永不失败。

**V2 剩余**：
- App 端离线阅读器（内嵌浏览/转 PDF 渲染，避免 WebView 复杂度回潮）
- 媒体孤儿清理（删除条目后比对 `/glean/media/`）+ 配额监控
- 批量收藏（多图/多文件）；网盘适配器 C 档（按用户量决策）
